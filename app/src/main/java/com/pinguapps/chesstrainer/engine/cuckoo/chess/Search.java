...
import com.pinguapps.chesstrainer.engine.cuckoo.chess.TranspositionTable.TTEntry;
...
public class Search {
    ...
    private final History ht;
    private final long[] posHashList;         // List of hashes for previous positions up to the last "zeroing" move.
    ...
    private final SearchTreeInfo[] searchTreeInfo;
            ...
            for (int mi = 0; mi < scMoves.length; mi++) {
                ...
                if (!firstIteration) {
                    if ((score > alpha) || (mi == 0)) {
                        ...
                        System.arraycopy(scMoves, 0, scMoves, 1, mi - 1 + 1);
                        scMoves[0] = tmp;
    ...
    public void scoreMove(Move m, int ply) {
            boolean isCapture = (pos.getPiece(m.to) != Piece.EMPTY) || (m.promoteTo != Piece.EMPTY);
            int score = 0;
            if (isCapture) {
                int seeScore = isCapture ? signSEE(m) : 0;
